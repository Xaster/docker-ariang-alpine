...
RUN apk upgrade --no-cache \
    && apk add --no-cache aria2 darkhttpd curl tzdata \
    && mkdir -p /aria2/conf /aria2/downloads /ariang \ 
    && ARIANG_VERSION=$(curl -sS --fail https://github.com/mayswind/AriaNg/releases | grep -o '/AriaNg/archive/[0-9.]*[.]tar[.]gz' | sed -e 's~^/AriaNg/archive/~~' -e 's~\.tar\.gz$~~' | sort -t '.' -k 1,1 -k 2,2 -k 3,3 -k 4,4 -g | tail -n 1) \
